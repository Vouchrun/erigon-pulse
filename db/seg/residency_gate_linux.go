...
// itself and nothing more. Capped at 8 pages so it never exceeds one io_uring
// warm (else the tail pages would be marked resident without being warmed).
var residencyWindow = min(dbg.EnvInt("RESIDENCY_WINDOW_PAGES", 1), 8) * pageSize
